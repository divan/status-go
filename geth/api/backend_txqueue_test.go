...
func (s *BackendTestSuite) TestCompleteMultipleQueuedTransactions() {
	...
	if s.backend.TxQueueManager().TransactionQueue().Count() != 0 {
...
func (s *BackendTestSuite) TestDiscardMultipleQueuedTransactions() {
	...
	discardTxs := func(txIDs []common.QueuedTxID) {
		...
		time.Sleep(1 * time.Second) // make sure that tx complete signal propagates
		for _, txID := range txIDs {
			if s.backend.TxQueueManager().TransactionQueue().Has(txID) {
...
func (s *BackendTestSuite) TestEvictionOfQueuedTransactions() {
	...
	if txQueue.Count() > node.DefaultTxQueueCap {
		s.Fail(fmt.Sprintf("transaction count should be %d (or %d): got %d", node.DefaultTxQueueCap, node.DefaultTxQueueCap-1, txQueue.Count()))
		return
	}
